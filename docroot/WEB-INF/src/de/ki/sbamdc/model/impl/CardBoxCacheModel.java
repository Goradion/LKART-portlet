/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

package de.ki.sbamdc.model.impl;

import aQute.bnd.annotation.ProviderType;

import com.liferay.portal.kernel.model.CacheModel;
import com.liferay.portal.kernel.util.HashUtil;
import com.liferay.portal.kernel.util.StringBundler;
import com.liferay.portal.kernel.util.StringPool;

import de.ki.sbamdc.model.CardBox;

import java.io.Externalizable;
import java.io.IOException;
import java.io.ObjectInput;
import java.io.ObjectOutput;

import java.util.Date;

/**
 * The cache model class for representing CardBox in entity cache.
 *
 * @author Simon Bastian, Alexander Mueller, Diego Castellanos
 * @see CardBox
 * @generated
 */
@ProviderType
public class CardBoxCacheModel implements CacheModel<CardBox>, Externalizable {
	@Override
	public boolean equals(Object obj) {
		if (this == obj) {
			return true;
		}

		if (!(obj instanceof CardBoxCacheModel)) {
			return false;
		}

		CardBoxCacheModel cardBoxCacheModel = (CardBoxCacheModel)obj;

		if (id == cardBoxCacheModel.id) {
			return true;
		}

		return false;
	}

	@Override
	public int hashCode() {
		return HashUtil.hash(0, id);
	}

	@Override
	public String toString() {
		StringBundler sb = new StringBundler(19);

		sb.append("{id=");
		sb.append(id);
		sb.append(", groupId=");
		sb.append(groupId);
		sb.append(", companyId=");
		sb.append(companyId);
		sb.append(", userId=");
		sb.append(userId);
		sb.append(", userName=");
		sb.append(userName);
		sb.append(", createDate=");
		sb.append(createDate);
		sb.append(", modifiedDate=");
		sb.append(modifiedDate);
		sb.append(", name=");
		sb.append(name);
		sb.append(", shared=");
		sb.append(shared);
		sb.append("}");

		return sb.toString();
	}

	@Override
	public CardBox toEntityModel() {
		CardBoxImpl cardBoxImpl = new CardBoxImpl();

		cardBoxImpl.setId(id);
		cardBoxImpl.setGroupId(groupId);
		cardBoxImpl.setCompanyId(companyId);
		cardBoxImpl.setUserId(userId);

		if (userName == null) {
			cardBoxImpl.setUserName(StringPool.BLANK);
		}
		else {
			cardBoxImpl.setUserName(userName);
		}

		if (createDate == Long.MIN_VALUE) {
			cardBoxImpl.setCreateDate(null);
		}
		else {
			cardBoxImpl.setCreateDate(new Date(createDate));
		}

		if (modifiedDate == Long.MIN_VALUE) {
			cardBoxImpl.setModifiedDate(null);
		}
		else {
			cardBoxImpl.setModifiedDate(new Date(modifiedDate));
		}

		if (name == null) {
			cardBoxImpl.setName(StringPool.BLANK);
		}
		else {
			cardBoxImpl.setName(name);
		}

		cardBoxImpl.setShared(shared);

		cardBoxImpl.resetOriginalValues();

		return cardBoxImpl;
	}

	@Override
	public void readExternal(ObjectInput objectInput) throws IOException {
		id = objectInput.readLong();

		groupId = objectInput.readLong();

		companyId = objectInput.readLong();

		userId = objectInput.readLong();
		userName = objectInput.readUTF();
		createDate = objectInput.readLong();
		modifiedDate = objectInput.readLong();
		name = objectInput.readUTF();

		shared = objectInput.readBoolean();
	}

	@Override
	public void writeExternal(ObjectOutput objectOutput)
		throws IOException {
		objectOutput.writeLong(id);

		objectOutput.writeLong(groupId);

		objectOutput.writeLong(companyId);

		objectOutput.writeLong(userId);

		if (userName == null) {
			objectOutput.writeUTF(StringPool.BLANK);
		}
		else {
			objectOutput.writeUTF(userName);
		}

		objectOutput.writeLong(createDate);
		objectOutput.writeLong(modifiedDate);

		if (name == null) {
			objectOutput.writeUTF(StringPool.BLANK);
		}
		else {
			objectOutput.writeUTF(name);
		}

		objectOutput.writeBoolean(shared);
	}

	public long id;
	public long groupId;
	public long companyId;
	public long userId;
	public String userName;
	public long createDate;
	public long modifiedDate;
	public String name;
	public boolean shared;
}